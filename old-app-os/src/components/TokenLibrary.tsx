...
export function TokenLibrary({ projectId }: TokenLibraryProps) {
  ...
  const tokens = useQuery(api.tokens.getTokens, {
  ...
  const handleDeleteToken = async (tokenId: string) => {
    if (
      !confirm(
        "Are you sure you want to delete this token? This will remove it from all documents.",
      )
    ) {
      return;
    }
                ...
                <div className="space-y-4 mt-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      This template will be used when generating images for
                      consistency
                    </p>
                  </div>
              ...
              <Select value={filterType} onValueChange={setFilterType}>
                ...
                <SelectContent>
                  <SelectItem value="all">
                    All Types ({tokens?.length || 0})
                  </SelectItem>
                  {TOKEN_TYPES.map((type) => {
                    const count =
                      tokens?.filter((t) => t.type === type.value).length || 0;
                    return (
                      <SelectItem key={type.value} value={type.value}>
      ...
      <div className="flex-1 overflow-auto p-4">
        {filteredTokens.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
            ...
            <p className="text-sm">
              Upload images or create tokens to start building your world
            </p>
          </div>
        ) : (
                ...
                >
                  <div className="p-4">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <div
                          className={`p-2 rounded-lg ${getTokenColor(token.type)}`}
                        >
                        ...
                        <div className="flex-1">
                          ...
                          <Badge variant="outline" className="text-xs mt-1">
                            {
                              TOKEN_TYPES.find((t) => t.value === token.type)
                                ?.label
                            }
                          </Badge>
                        </div>
