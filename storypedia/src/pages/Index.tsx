...
import { categoryLabels, categoryIcons, pagesByCategory, type PageCategory } from '@/data/wiki';
    ...
    <div className="flex-1 px-6 py-8 max-w-4xl">
      ...
      <div className="mb-10">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h1 className="font-mono text-4xl font-bold text-wiki-heading">Storypedia</h1>
        </div>
        <p className="font-serif text-lg text-foreground/80 leading-relaxed max-w-xl">
          Welcome to the encyclopedia of my creative universe. Explore stories, characters, 
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        {categoryOrder.map(cat => {
          ...
          return (
            <div
              key={cat}
              className="rounded-lg border border-border bg-card p-5 transition-all hover:border-primary/30 hover:shadow-[0_0_20px_hsl(155,100%,50%,0.05)]"
            >
      ...
      <div className="mt-10 pt-6 border-t border-border">
        <p className="font-mono text-xs text-muted-foreground">
          <span className="text-primary">{Object.values(pagesByCategory).flat().length}</span> pages ·{' '}
          <span className="text-primary">{categoryOrder.length}</span> categories ·{' '}
